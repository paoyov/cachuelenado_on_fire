...
// Helper to find selected client from conversations
$cliente = null;
if (!empty($cliente_id) && !empty($conversaciones)) {
            ...
            <div class="conversations">
                <?php if (!empty($conversaciones)): ?>
                    <?php foreach ($conversaciones as $c): ?>
                        <a class="conv-item" href="<?php echo BASE_URL; ?>maestro/mensajes?cliente_id=<?php echo $c['cliente_id']; ?>">
        ...
        <main class="chat-main">
            <div class="chat-top">
                <?php if (!empty($cliente)): ?>
                    <div class="maestro-info">
                        <img src="<?php echo UPLOAD_URL . ($cliente['foto_perfil'] ?: 'perfiles/default.png'); ?>" alt="" class="maestro-thumb"/>
                        <div>
                            <strong><?php echo htmlspecialchars($cliente['nombre_completo']); ?></strong>
                        </div>
                    </div>
                <?php else: ?>
                ...
                <div class="chat-box" id="chat-box">
                    <?php if (!empty($mensajes)): ?>
                        <?php foreach ($mensajes as $m): ?>
                            <div class="chat-message <?php echo $m['enviado_por'] === 'maestro' ? 'me' : 'other'; ?>" data-id="<?php echo $m['id']; ?>">
                                <?php if ($m['enviado_por'] !== 'maestro'): ?><div class="msg-avatar"><img src="<?php echo UPLOAD_URL . ($cliente['foto_perfil'] ?? 'perfiles/default.png'); ?>"/></div><?php endif; ?>
                                <div class="msg-body">
            ...
            <div class="chat-input">
                ...
                <form id="sendForm" enctype="multipart/form-data">
                    ...
                    <?php endif; ?>
                    <div class="input-row">
                        <textarea name="mensaje" id="mensaje" placeholder="<?php echo !empty($cliente) ? 'Escribe un mensaje...' : 'Selecciona una conversación para escribir...'; ?>" <?php echo empty($cliente) ? 'disabled' : ''; ?>></textarea>
                        <div class="input-actions">
                            <label class="attach-btn">
                                <input type="file" name="adjunto" id="adjunto" accept="image/*,video/*" <?php echo empty($cliente) ? 'disabled' : ''; ?> />
                                📎
                            </label>
                            <button type="button" id="sendBtn" class="send-btn" <?php echo empty($cliente) ? 'disabled' : ''; ?>>➤</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <aside class="chat-right">
            <?php if (!empty($cliente)): ?>
                <div class="profile-card">
                    <img src="<?php echo UPLOAD_URL . ($cliente['foto_perfil'] ?: 'perfiles/default.png'); ?>" class="profile-pic" />
                    <h4><?php echo htmlspecialchars($cliente['nombre_completo']); ?></h4>
                    <p class="muted">Cliente</p>
                </div>
            <?php else: ?>
                <div class="profile-card muted">Selecciona una conversación para ver detalles del cliente.</div>
            <?php endif; ?>
        </aside>
    </div>
</div>
...
(() => {
    ...
    const clienteId = document.querySelector('input[name="cliente_id"]') ? document.querySelector('input[name="cliente_id"]').value : null;

    const BASE = '<?php echo BASE_URL; ?>';
    const WS_HOST = 'ws://localhost:8080';
    const userId = <?php echo (int)$maestro['id']; ?>;
    const userType = 'maestro';

    // Avatar for the other person (cliente)
    const otherAvatar = "<?php echo UPLOAD_URL . ($cliente['foto_perfil'] ?? 'perfiles/default.png'); ?>";
    ...
    function renderMessage(m){
        ...
        const isMe = (m.enviado_por === 'maestro');
        ...
        ws.addEventListener('message', (ev) => {
            try{
                ...
                if (data.action === 'new_message' && data.mensaje) {
                    ...
                    const belongs = (m.maestro_id == userId && m.cliente_id == clienteId) || (m.maestro_id == clienteId && m.cliente_id == userId);
    ...
    sendBtn.addEventListener('click', async (e) => {
        e.preventDefault();
        if (!clienteId) return alert('Selecciona una conversación');
        ...
        const payload = { action: 'send_message', sender_id: userId, sender_type: userType, receiver_id: parseInt(clienteId), mensaje: txt || null, tipo: tipo, adjunto: adjPath };
    ...
    (async function loadInitial(){
        if (!clienteId) return;
        try{
            const res = await fetch(`${BASE}api/mensajes/obtener?cliente_id=${clienteId}&maestro_id=${userId}`);
